...
if ( isset( $_SESSION['email'] ) ) {
    ...
    if ( isset( $_POST['submit'] ) ) {
        ...
        if ( $old == $new ) {
            echo "<script>alert('Old and new password can not be same');window.location.href = 'staff_change_pass.php';</script>";
        } else {
            ...
            if ( $old == $result['pass']) {
                ...
                if ( $run2 ) {
                    session_destroy();
                    echo "<script>alert('You have successfully changed your password. Now please Login again');window.location.href ='staff_login_main.php';</script>";
                }
            } else {
                echo "<script>alert('Please recheck your old password');window.location.href ='staff_change_pass.php';</script>";
            }
        }
    }
    ?>
