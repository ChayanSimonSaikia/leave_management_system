...
if ( isset( $_SESSION['email'] ) ) {
    ...
    <link rel="stylesheet" href="css/change_color2.css">
    </head>
    <script src = 'js/validation.js'></script>
    <body>
    <?php include( 'nav_bar_staff.php' );
    include( 'vertical_bar_staff.php' );
    ?>
    ...
    if ( isset( $_POST['submit'] ) ) {
        ...
        if ( $old == $new ) {
            echo "<script>alert('Old email and new email can not be same');window.location.href = 'staff_settings.php';</script>";
        } else {
            if ( $confirm == $result['pass'] && $old == $result['email'] ) {
                ...
                if ( $run2 ) {
                    session_destroy();
                    echo "<script>alert('You have successfully changed your email. Now please Login again');window.location.href ='staff_login_main.php';</script>";
                }
            } else {
                echo "<script>alert('Please recheck your old email and password');window.location.href ='staff_settings.php';</script>";
            }
        }
    }
    ?>
